...
#     print("No vector store. Build a new one.")
...
VECTOR_DIR = Path("vector_store")
FAISS_INDEX_PATH = VECTOR_DIR / "faiss_index" / "index.faiss"
METADATA_PATH = VECTOR_DIR / "faiss_index" / "metadata.pkl"  # Store document metadata

TOP_K = 5
MAX_CONTEXT_CHARS = 12000
SIMILARITY_DISTANCE_MAX = 1.5  # Adjusted for L2 distance
...
def load_faiss_index():
    ...
    print(f"✅ Loaded FAISS index with {faiss_index.ntotal} vectors (dim={faiss_index.d})")
    
    # Load metadata (documents, sources, etc.)
    if METADATA_PATH.exists():
        ...
        print("   Creating empty metadata structure...")
        document_metadata = []


# =========================
# Simple Embedding Using Groq (Fallback)
# =========================
def get_simple_embedding(text: str) -> np.ndarray:
    """
    Create a simple embedding using Groq API.
    This is a fallback method - ideally you'd use the same model used for indexing.
    """
    try:
        # Use Groq to generate a semantic representation
        # Note: This won't match your original embeddings exactly
        # Better solution: Use OpenAI embeddings API or Cohere embeddings
        
        response = groq_client.chat.completions.create(
            model=GROQ_MODEL,
            messages=[
                {"role": "system", "content": "Extract key agricultural terms from this text."},
        ...
        # Convert response to a simple vector (this is crude but works)
        keywords = response.choices[0].message.content.lower()
        
        # Create a simple bag-of-words style vector
        # This is a placeholder - see alternative below
        vector = np.zeros(faiss_index.d, dtype=np.float32)
        words = keywords.split()
        for i, word in enumerate(words[:faiss_index.d]):
            vector[i] = len(word) / 10.0  # Simple hash-like encoding
        ...
        vector = np.random.rand(faiss_index.d).astype(np.float32)
        return vector / np.linalg.norm(vector)


# =========================
# Better Alternative: Use Keyword Matching
# =========================
def keyword_search(query: str, k: int = TOP_K) -> List[int]:
    """
    ...
    for idx, doc in enumerate(document_metadata):
...
# =========================
def retrieve_context(query: str, k: int = TOP_K) -> Tuple[str, List[str]]:
    """Retrieve relevant context using FAISS or keyword search."""
    if not query or not query.strip():
        return "", []
    ...
    try:
        # Method 1: Try simple keyword matching first (more reliable without embeddings)
        if document_metadata:
            print("🔍 Using keyword search...")
            indices = keyword_search(q, k=k)
            ...
            if indices:
                ...
                for idx in indices:
                    ...
                    doc = document_metadata[idx]
                    text = doc.get('text', '').strip()
                    
                    if not text or is_junk_content(text):
                        continue
        ...
        # Method 2: Fallback to FAISS with simple embedding
        print("🔍 Using FAISS search...")
        query_vector = get_simple_embedding(q)
        ...
        for idx, dist in zip(indices[0], distances[0]):
            if idx < 0 or idx >= len(document_metadata):
                continue
            
            if dist > SIMILARITY_DISTANCE_MAX:
                continue
